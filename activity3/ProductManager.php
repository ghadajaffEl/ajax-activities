<?php
/**
 * Created by PhpStorm.
 * User: jghada
 * Date: 03/10/2019
 * Time: 14:48
 */

class ProductManager
{
    private $dnsh = "mysql:host=localhost;dbname=workshop_ajax";
    private $user = "root";
    private $pass = "";
    private $db = null;


    public function __construct()
    {
        try {
            $this->db = new PDO($this->dnsh, $this->user, $this->pass);

        } catch (Exception $e) {
            die('Erreur : ' . $e->getMessage());
        }


    }

    public function addProduct()
    {
        if (isset($_POST['name']) && isset($_POST['price'])) {
            $name = htmlspecialchars($_POST['name']);
            $price = $_POST['price'];
            $description = $_POST['description'];
            $req = $this->db->prepare('INSERT INTO product(name, price, description) VALUES(:name, :price, :description)');
            $req->execute(array(
                'name' => $name,
                'price' => $price,
                'description' =>$description

            ));
            $lastId=$this->db->lastInsertId();

            $nbProducts = count($this->productList());
            echo json_encode(array('name' => $name, 'price' => $price, 'nb' => $nbProducts,'id'=>$lastId));
        } else {

            echo json_encode(array('success' => 0));
        }

    }

    public function productList()
    {

        $data = [];
        $response = $this->db->query('SELECT * FROM Product');

        while ($donnees = $response->fetch()) {

            $product = [];
            $product['name'] = $donnees['name'];
            $product['price'] = $donnees['price'];
            $product['id'] = $donnees['id'];
            $data[] = $product;
        }
        $response->closeCursor();

        return $data;
    }

    public function getProduct()
    {
        $id = $_GET['id'];
        $req = $this->db->prepare('SELECT * FROM product WHERE id = ?');
       $req->execute([$id]);

        while ($donnees = $req->fetch())
        {
            echo '<h3>' . $donnees['name'] .'</h3><br><p>Description: '.$donnees['description'].'</p> <br><small class="text-muted">Prix: '.$donnees['price'].'</small>';
        }

        $req->closeCursor();

    }


}